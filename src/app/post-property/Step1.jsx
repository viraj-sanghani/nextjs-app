...
import { call, getCity, getLocality, saveProperty } from "@/services/api";
import {
  setStep,
  setPropType,
  setPropId,
...
function Step1({ draftData }) {
  ...
  const { step } = useSelector((state) => state.propertyForm);
  ...
  const handleSubmit = async (values, { setSubmitting, setErrors }) => {
    if (!values.latitude || !values.longitude) {
      return error("Select Your Location");
    }

    try {
      const res = await call(saveProperty(values));
          ...
          <LocationPin
            ...
            search={
              form.values?.locality
                ? `${form.values?.locality} ${form.values?.city}`
